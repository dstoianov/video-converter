...
from src import write_to_csv_file, logger

# path = '/media/funker/3/FOTO/2017/2017-Samsung A5 and S7 Denys/2017-04/'
path = '/media/funker/3/FOTO/2018/'

csv_file_name = 'movies_all_mp4_2017.csv'

files = []
files_ext = {}
skipped_file_extensions = ['nfo', 'ini', 'jpg', 'txt', 'db', 'png', 'jpeg']
...
def read_files():
    ...
    for r, d, f in os.walk(path):
        for file in f:
            ...
            if 'documentary movies' not in full_path:
...
def convert_videos(files):
    ...
    for filename in files:
        ...
        if not os.path.isfile(output_file):
            subprocess.run(shlex.split(command), stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                           universal_newlines=True)
        end = time.time()
        size_mb = round(os.path.getsize(output_file) / 1024 / 1024, 2)
        logger.info("\tElapsed time '%.4s' sec", end - start)
        logger.info("\tNew size '%sMb', ratio '%.4s'", size_mb, filename['size'] / size_mb)
